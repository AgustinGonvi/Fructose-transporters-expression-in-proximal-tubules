---
title: "Transporter Location - Differential Gene Expression (DESeq)"
...
```{r load_raw_data_preparation}
# factorize metadata
metadata$PTSegment <- factor(metadata$PTSegment)

# load saved matrix from SubRead
raw_count <- read.table("./../00_Inputs/raw_count_matrix.csv", 
                       header = TRUE, sep = ",") 
...
NormalizeDDS <- function(dds) {
  ...
  dataExp_N <- counts(dds, normalized = T)
...
# write.csv(metadata,"./../data/Metadata_newgroup.csv",row.names = FALSE)
write.table(S1_S2,"./../02_Results/DEG_S1_S2.csv",sep = ",",row.names = T)
write.table(S1_S3,"./../02_Results/DEG_S1_S3.csv",sep = ",",row.names = T)
write.table(S2_S3,"./../02_Results/DEG_S2_S3.csv",sep = ",",row.names = T)
```

# Export the Results for Log Tranformation
```{r export_results_for_log}
...
# export the normalizaed matrix for log transformation
